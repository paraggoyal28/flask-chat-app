...
client = MongoClient(
    "mongodb+srv://<user>:<password>@mychatcluster.4q5dz4s.mongodb.net/?retryWrites=true&w=majority&appName=MyChatCluster",
...
def save_room(room_name, created_by):
    ...
    add_room_member(room_id, room_name, created_by, created_by, True)
    return room_id
...
def remove_room_members(room_id, usernames):
    room_members_collection.delete_many({'_id': {'$in': [{'room_id':
                                                              ObjectId(room_id),
...
def get_messages(room_id, page=0):
    ...
    messages = list(messages_collection.find({'room_id': room_id}).sort('created_at', DESCENDING).skip(offset).limit(MESSAGE_FETCH_LIMIT))
